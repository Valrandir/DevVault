...
int Path::Node::MakeKey(const Point& pt, int WorldWidth)
{
	return pt.x + pt.y * WorldWidth;
}
...
Path::Node::Node(const Point& Position, const Point& Goal, Node* PrevNode, int WorldWidth) :
	Position(Position),
	PrevNode(PrevNode),
	Key(MakeKey(Position, WorldWidth)),
	NodeState(Node::NodeStateEnum::Open)
{
	Eval(Position, Goal, PrevNode, PathSpan, GoalSpan, Score);
}

Path::Node::Node(const Point& Position, int PathSpan, int GoalSpan, int Score, Node* PrevNode, int WorldWidth) :
	Position(Position),
	PathSpan(PathSpan),
	GoalSpan(GoalSpan),
	Score(Score),
